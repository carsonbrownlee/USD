...
/// \class HdOSPRayRTCBufferAllocator
///
...
// ----------------------------------------------------------------------
// The classes below implement the HdOSPRayPrimvarSampler interface for
...
/// \class HdOSPRayConstantSampler
///
/// This class implements the HdOSPRayPrimvarSampler interface for primvars
...
class HdOSPRayConstantSampler : public HdOSPRayPrimvarSampler {
public:
    /// Constructor.
    /// \param name The name of the primvar.
    /// \param value The buffer data for the primvar.
    HdOSPRayConstantSampler(TfToken const& name,
                            VtValue const& value)
        : _buffer(name, value)
        , _sampler(_buffer) {}
...
private:
    ...
    HdOSPRayBufferSampler const _sampler;
};

/// \class HdOSPRayUniformSampler
///
/// This class implements the HdOSPRayPrimvarSampler interface for primvars
/// with "uniform" interpolation mode. This means that the buffer has one
/// item per authored face. For unrefined meshes, HdOSPRay will convert
...
class HdOSPRayUniformSampler : public HdOSPRayPrimvarSampler {
public:
    /// Constructor.
    /// \param name The name of the primvar.
    /// \param value The buffer data for the primvar.
    ...
    ///                        face index.
    HdOSPRayUniformSampler(TfToken const& name,
                           VtValue const& value,
...
/// \class HdOSPRayTriangleVertexSampler
///
/// This class implements the HdOSPRayPrimvarSampler interface for primvars on
...
class HdOSPRayTriangleVertexSampler : public HdOSPRayPrimvarSampler {
public:
    /// Constructor.
    /// \param name The name of the primvar.
    /// \param value The buffer data for the primvar.
    ...
    HdOSPRayTriangleVertexSampler(TfToken const& name,
                                  VtValue const& value,
    ...
    /// they are interpolated as follows, per OSPRay specification:
...
/// \class HdOSPRayTriangleFaceVaryingSampler
///
/// This class implements the HdOSPRayPrimvarSampler interface for primvars on
...
class HdOSPRayTriangleFaceVaryingSampler : public HdOSPRayPrimvarSampler {
public:
    ...
    HdOSPRayTriangleFaceVaryingSampler(TfToken const& name,
                                       VtValue const& value,
                                       HdMeshUtil &meshUtil)
    ...
    /// the three vertices, they are interpolated as follows, per OSPRay
    /// specification:
    /// t_uv = (1-u-v)*t0 + u*t1 + v*t2
    /// 
    /// \param element The element index to sample.
...
/// \class HdOSPRaySubdivVertexSampler
///
/// This class implements the HdOSPRayPrimvarSampler interface for primvars on
...
class HdOSPRaySubdivVertexSampler : public HdOSPRayPrimvarSampler {
public:
    ...
    HdOSPRaySubdivVertexSampler(TfToken const& name,
                                VtValue const& value,
                                RTCScene meshScene,
                                unsigned meshId,
                                HdOSPRayRTCBufferAllocator *allocator);

    /// Destructor. Frees the embree user vertex buffer.
    virtual ~HdOSPRaySubdivVertexSampler();
...
private:
    ...
    RTCScene _meshScene;
    unsigned _meshId;
    HdOSPRayRTCBufferAllocator *_allocator;
};
