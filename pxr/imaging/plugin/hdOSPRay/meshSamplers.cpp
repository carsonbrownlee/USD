...
RTCBufferType
HdOSPRayRTCBufferAllocator::Allocate()
{
...
void
HdOSPRayRTCBufferAllocator::Free(RTCBufferType buffer)
{
    _bitset.reset(buffer - RTC_USER_VERTEX_BUFFER);
}

// HdOSPRayConstantSampler

bool
HdOSPRayConstantSampler::Sample(unsigned int element, float u, float v,
    void* value, int componentType, short numComponents) const
{
    return _sampler.Sample(0, value, componentType, numComponents);
}

// HdOSPRayUniformSampler

bool
HdOSPRayUniformSampler::Sample(unsigned int element, float u, float v,
    void* value, int componentType, short numComponents) const
{
...
// HdOSPRayTriangleVertexSampler

bool
HdOSPRayTriangleVertexSampler::Sample(unsigned int element, float u, float v,
    void* value, int componentType, short numComponents) const
{
    if (element >= _indices.size()) {
        return false;
    }
    HdOSPRayTypeHelper::PrimvarTypeContainer corners[3];
    ...
    // OSPRay specification of triangle interpolation:
...
// HdOSPRayTriangleFaceVaryingSampler

bool
HdOSPRayTriangleFaceVaryingSampler::Sample(unsigned int element, float u,
...
/* static */ VtValue
HdOSPRayTriangleFaceVaryingSampler::_Triangulate(TfToken const& name,
...
// HdOSPRaySubdivVertexSampler

HdOSPRaySubdivVertexSampler::HdOSPRaySubdivVertexSampler(TfToken const& name,
    ...
    , _allocator(allocator)
{
    ...
    if (_buffer.GetGLComponentDataType() != GL_FLOAT) {
        TF_CODING_ERROR("OSPRay subdivision meshes only support float-based"
    ...
    if (_embreeBufferId == static_cast<RTCBufferType>(-1)) {
        TF_CODING_ERROR("OSPRay subdivision meshes only support %d primvars"
...
bool
HdOSPRaySubdivVertexSampler::Sample(unsigned int element, float u, float v,
    void* value, int componentType, short numComponents) const
{
